  ...
  <div>
    <form @submit.prevent="submitForm()" method="POST" class="form_admin_create_item">
      <div class="form_block form_admin_create_block">
        <input type="input" class="form_block__input has-val" name="name" v-model="form.name" required autocomplete="name" autofocus>
      ...
      <div class="form_block form_admin_create_block">
        <input type="email" class="form_block__input has-val" name="email" v-model="form.email" required autocomplete="email">
      ...
      <div class="form_block form_admin_create_block">
        <input type="password" class="form_block__input" name="password" v-model="form.password" autocomplete="password">
        <span class="form_block__input--focus" data-placeholder="Пароль (укажите если нужно изменить)"></span>
      </div>
    ...
    <form @submit.prevent="removeUser()" method="POST" class="form_admin_delete_item">
...
export default {
  ...
  data(){
    return {
      routeSave: document.querySelector('meta[name=route-update]').getAttribute('content'),
      routeRemove: document.querySelector('meta[name=route-remove]').getAttribute('content')
    }
  },
  methods: {
    removeUser(){
  ...
  mounted(){
    let teacher = window.teacherData;
    this.form.name = teacher.name;
    this.form.email = teacher.email;
    this.form.password = teacher.password;
    for(let groupData of teacher.groups){
      this.form.groups.push({
        'group': groupData.group,
        'courseWork': groupData.courseWork,
        'discipline': groupData.discipline,
        'formControl': groupData.formControl,
        'groupName': groupData.groupName,
        'attestations': groupData.attestations
      });
    }
  }
}
